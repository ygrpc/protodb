...
// DbTableQuery executes a TableQueryReq and calls fn once for every matching row.
// msg is used as the row prototype; each callback receives a newly allocated message.
func DbTableQuery(db sqldb.DB, msg proto.Message, req *protodb.TableQueryReq, fn func(proto.Message) error) error {
	return DbTableQueryContext(context.Background(), db, msg, req, fn)
}

// DbTableQueryContext is the context-aware variant of DbTableQuery.
func DbTableQueryContext(ctx context.Context, db sqldb.DB, msg proto.Message, req *protodb.TableQueryReq, fn func(proto.Message) error) error {
	return dbTableQueryContext(ctx, db, msg, req, "", nil, fn)
}

// DbTableQueryWithPermission executes a TableQueryReq with an additional trusted
// permission predicate and values. permissionSQL must be generated by the caller,
// not taken directly from an untrusted request.
func DbTableQueryWithPermission(db sqldb.DB, msg proto.Message, req *protodb.TableQueryReq, permissionSQL string, permissionArgs []any, fn func(proto.Message) error) error {
	return DbTableQueryWithPermissionContext(context.Background(), db, msg, req, permissionSQL, permissionArgs, fn)
}

// DbTableQueryWithPermissionContext is the context-aware permission-aware variant.
func DbTableQueryWithPermissionContext(ctx context.Context, db sqldb.DB, msg proto.Message, req *protodb.TableQueryReq, permissionSQL string, permissionArgs []any, fn func(proto.Message) error) error {
	return dbTableQueryContext(ctx, db, msg, req, permissionSQL, permissionArgs, fn)
}

func dbTableQueryContext(ctx context.Context, db sqldb.DB, msg proto.Message, req *protodb.TableQueryReq, permissionSQL string, permissionArgs []any, fn func(proto.Message) error) error {
	if msg == nil {
		return fmt.Errorf("table query message is nil")
	}
	if req == nil {
		return fmt.Errorf("table query request is nil")
	}
	if fn == nil {
		return fmt.Errorf("table query row callback is nil")
	}

	msgDesc := msg.ProtoReflect().Descriptor()
	sqlStr, sqlVals, err := TableQueryBuildSql(db, msgDesc, req, permissionSQL, permissionArgs)
	if err != nil {
		return err
	}
	rows, err := db.QueryContext(ctx, sqlStr, sqlVals...)
	if err != nil {
		return err
	}
	defer rows.Close()
	...
	rowScanner, err := NewDbRowScanner(rows, msg, fieldNames, msgFieldsMap)
	if err != nil {
		return err
	}
	for rows.Next() {
		rowMsg := msg.ProtoReflect().New().Interface()
		if err := rowScanner.Scan(rows, rowMsg); err != nil {
			return err
		}
		if err := fn(rowMsg); err != nil {
			return err
		}
	}
	return rows.Err()
...
// DbTableQueryList executes a TableQueryReq and collects all matching rows.
func DbTableQueryList(db sqldb.DB, msg proto.Message, req *protodb.TableQueryReq) ([]proto.Message, error) {
	result := make([]proto.Message, 0)
	err := DbTableQuery(db, msg, req, func(row proto.Message) error {
