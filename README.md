...
**注意**: 所有数组字段均强制设为 `NOT NULL`，默认值为空集合 (Postgres: 标量 `'{}'`/消息 `'[]'`, MySQL: `CAST('[]' AS JSON)`, SQLite: `'[]'`)。
...
**MySQL 支持**: 使用 `JSON_CONTAINS`, `JSON_OVERLAPS` (8.0.17+), `JSON_LENGTH` 实现上述语义。对 `repeated message` 的 `WOP_CONTAINS`，会将传入 JSON 对象包装为单元素 JSON 数组后参与匹配。
...
`protodb` 提供了 `ddl.DbCreateSQL` 与 `ddl.DbMigrateTable`，可根据 Proto 定义生成建表/迁移 SQL。当前 PostgreSQL、MySQL、SQLite 都支持这两条 DDL 路径；其中 MySQL 的数组查询依赖 `JSON_OVERLAPS`，建议使用 MySQL 8.0.17+。
